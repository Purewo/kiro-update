...
export interface ProxyAccount {
  ...
  email?: string
  /**
   * Whether this account participates in the pool. Web build uses this to
   * keep disabled accounts in the snapshot (UI listing) without routing
   * traffic to them. Undefined / true = enabled.
   */
  enabled?: boolean
  accessToken: string
  refreshToken?: string
...
export interface ProxyStats {
  ...
  cacheWriteTokens: number
  reasoningTokens: number
  // Throttle-retry tracking. retriedRequests = number of *requests* that hit
  // a throttle and recovered via account switch (counted once per request,
  // regardless of how many switches it took). totalAccountSwitches = sum of
  // all throttle-driven switches across all requests; the difference between
  // this and retriedRequests is the avg switches per recovered request.
  ...
  // Rolling 24h usage events. Each entry stores only the bare minimum needed
  // to derive 24h token / credit / request totals on the read side. The array
  // is pruned to the last 24h on every push, so it never grows past one day's
  // worth of traffic — even at 6 acc × peak 1 req/s that's ≤ 6×86400 = 518K
  // entries × 32 bytes ≈ 16 MB worst case; realistic usage is much smaller.
  usage24h: UsageEvent[]
}
...
export interface RequestLog {
  ...
  success: boolean
  error?: string
  // HTTP-style status code: 200 on success, otherwise parsed from upstream
  // error (402 quota, 403 token, 429 throttle, 5xx server, etc.)
  statusCode?: number
  // attempts = total tries this request consumed (1 = first-shot success).
  // accountChain = ordered list of accountIds tried, last one is the final
  // account that produced the result. Only set when attempts > 1.
  attempts?: number
  accountChain?: string[]
}
