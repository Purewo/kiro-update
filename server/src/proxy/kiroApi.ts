...
const OS_RELEASE = (() => { try { return os.release() } catch { return '10.0.0' } })()
...
/**
 * 反代调 Kiro API 时使用的 profileArn 决策（异步）。
 * - 已缓存真实 ARN：直接返回。
 * - Social/Github/Google：返回固定 social profile ARN（后端接受）。
 * - 企业 IdC（authMethod=idc/IdC，且 provider≠Github/Google）：调 ListAvailableProfiles
 *   主动获取真实 ARN 并写回到 account.profileArn 供后续复用。
 * - BuilderId：保留占位符（保持原 IDE 兼容行为；流式路径会剥离）。
 *
 * 关键事实：后端对带 BuilderId 占位符 ARN 的企业账号请求会判 token invalid 返回 403。
 * 因此企业账号必须用真实 ARN，或干脆不带（不带也 200，已实测）。
 */
async function resolveProfileArn(account: ProxyAccount, signal?: AbortSignal): Promise<string | undefined> {
  ...
  // 企业 IdC：尝试拉真实 ARN
  if (account.authMethod?.toLowerCase() === 'idc' || account.authMethod === 'external_idp') {
    try {
      const real = await ensureRealProfileArn(account, signal)
      if (real) return real
    } catch (err) {
      console.warn('[KiroAPI] ensureRealProfileArn failed:', (err as Error).message)
    }
    // 拉不到真 ARN：宁可不带也别毒化 token
    return undefined
  }
  // BuilderId 等其它：保持原占位符
  return KIRO_BUILDER_ID_PLACEHOLDER_ARN
}

// 进程内并发去重 + 缓存：同一账号同时只发一个 ListAvailableProfiles。
const profileArnFetchInflight = new Map<string, Promise<string | undefined>>()

async function ensureRealProfileArn(account: ProxyAccount, signal?: AbortSignal): Promise<string | undefined> {
  const inflight = profileArnFetchInflight.get(account.id)
  if (inflight) return inflight
  const p = (async () => {
    const profiles = await fetchAvailableProfiles(account, signal)
    const arn = profiles.find(p => p.arn && p.arn.trim())?.arn
    if (arn) {
      // 写回到账号对象（accountPool 持的是同一引用），后续请求直接命中第一分支。
      account.profileArn = arn
    }
    return arn
  })()
  profileArnFetchInflight.set(account.id, p)
  try {
    return await p
  } finally {
    profileArnFetchInflight.delete(account.id)
  }
}

interface KiroProfileEntry {
  arn: string
  profileArn?: string
  region?: string
}

// 调 POST https://codewhisperer.us-east-1.amazonaws.com/ListAvailableProfiles
// 用于企业 IdC 账号获取真实 profile ARN。
async function fetchAvailableProfiles(account: ProxyAccount, signal?: AbortSignal): Promise<KiroProfileEntry[]> {
  const url = 'https://codewhisperer.us-east-1.amazonaws.com/ListAvailableProfiles'
  const headers = getAuthHeaders(account, KIRO_ENDPOINTS[0])
  const res = await fetchWithProxy(url, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({ maxResults: 10 }),
    signal,
  } as RequestInit, account)
  const text = await res.text()
  if (!res.ok) {
    throw new Error(`ListAvailableProfiles HTTP ${res.status}: ${text.slice(0, 200)}`)
  }
  try {
...
function getModelCacheKey(account: ProxyAccount): string {
  // Cache key uses whatever ARN is currently on the account; ListAvailableProfiles
  // write-back will populate account.profileArn for IdC accounts after first use.
  return `${account.id}:${account.region || 'us-east-1'}:${account.profileArn ?? 'no-arn'}`
}
...
function getAuthHeaders(account: ProxyAccount, _endpoint: typeof KIRO_ENDPOINTS[0]): Record<string, string> {
  ...
  // kiro-go uses agent-mode=vibe across all account types (BuilderId, Social,
  // IdC). The earlier branching (`isIDC ? VIBE : SPEC`) in this port caused
  // AWS to reject IdC requests with "Your subscription does not support this
  // application." — kiro-go's UA + agent-mode combo is what the AWS-side
  // authorizer actually expects from the Kiro IDE client.
  const agentMode = AGENT_MODE_VIBE

  // Same fix for User-Agent: kiro-go always sends the KiroIDE UA, never the
  // AmazonQ-For-CLI rust UA. The CLI UA only applies to OIDC token-refresh
  // calls, not the streaming /generateAssistantResponse endpoint, regardless
  // of authMethod. Sending the CLI UA here trips AWS's app-id check.
...
export async function callKiroApiStream(
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      const resolvedArn = await resolveProfileArn(account, signal)
...
export async function fetchKiroModels(account: ProxyAccount, signal?: AbortSignal): Promise<KiroModel[]> {
  ...
  try {
    do {
      ...
      const arnForModels = await resolveProfileArn(account, signal)
...
export async function fetchAvailableSubscriptions(account: ProxyAccount): Promise<SubscriptionListResponse> {
  ...
  const profileArn = await resolveProfileArn(account)
