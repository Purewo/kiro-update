...
import { callKiroApiStream, callKiroApi, fetchKiroModels, setModelContextWindow, type KiroModel, clearAllCaches } from './kiroApi'
...
export class ProxyServer {
  ...
  private getTlsOptions(): https.ServerOptions {
    ...
    if (tls.cert && tls.key) {
      ...
      try {
        ...
        const result = ensureProxySelfSignedCert(getDataDir(), hostnames)
  ...
  getSelfSignedCertInfo(): import('./selfSignedCert').ProxySelfSignedCert | null {
    try {
      return ensureProxySelfSignedCert(getDataDir(), [this.config.host || '127.0.0.1'])
    } catch (err) {
  ...
  regenerateSelfSignedCert(): import('./selfSignedCert').ProxySelfSignedCert | null {
    try {
      ...
      return ensureProxySelfSignedCert(getDataDir(), [this.config.host || '127.0.0.1'], true)
    } catch (err) {
  ...
  // 获取统计信息
  getStats(): ProxyStats {
    // Prune 24h window before returning so the snapshot is always fresh.
    this.pruneUsage24h()
    ...
    return {
      ...
      totalAccountSwitches: this.stats.totalAccountSwitches,
      ...
      usage24h: this.stats.usage24h,
    }
  }

  // Drop usage events older than 24h. Called on every read so consumers
  // never see stale entries; called on every write so the array can't grow
  // beyond ~24h of traffic regardless of read frequency.
  private pruneUsage24h(): void {
    const cutoff = Date.now() - 24 * 60 * 60 * 1000
    const arr = this.stats.usage24h
    if (arr.length === 0 || arr[0].timestamp >= cutoff) return
    // Binary search for the first kept index — the array is monotonic in time.
  ...
  resetRequestStats(): void {
    ...
    this.stats.totalAccountSwitches = 0
    this.stats.usage24h = []
  ...
  recordApiKeyUsage(apiKeyId: string, credits: number, inputTokens: number, outputTokens: number, model?: string, path?: string): void {
    ...
    // Defensive init: web build's apiKey can arrive without `.usage` if a
    // previous version persisted it without that struct, or if someone hand-
    // edited the config file. Initializing here keeps recordApiKeyUsage
    // idempotent regardless of how the apiKey got into config.apiKeys.
    if (!apiKey.usage) {
      apiKey.usage = {
        totalRequests: 0,
        totalCredits: 0,
  ...
  private async handleClaudeStream(
    ...
    // Pre-output throttle retry state — passed across re-entries when a 429
    // is hit before any text/tool delta has been sent. Each retry picks the
    // next account in the pool excluding everyone we've already tried this
    // request. Once a delta has been written to res, retries stop (we can't
    // unroll output without confusing the client).
    triedAccountIds: Set<string> = new Set([account.id]),
    poolPasses: number = 0,
  ): Promise<void> {
    if (!headersSent) {
      res.writeHead(200, {
    ...
    return new Promise((resolve) => {
      callKiroApiStream(
        ...
        async (usage) => {
          ...
          this.events.onTokensUpdate?.(this.stats.inputTokens, this.stats.outputTokens)
          this.accountPool.recordSuccess(account.id, usage.inputTokens + usage.outputTokens)
          // Only count real upstream cache usage. Kiro backend itself doesn't
          // do prompt caching, so its `cacheReadTokens`/`cacheWriteTokens` come
          // back as 0; promptCacheTracker fabricates plausible numbers for the
          // Claude-compat response so clients see a sensible cache_creation/
          // cache_read split, but those values are not real billed tokens.
          // Surfacing them in the dashboard would be a lie ("缓存写 327K" while
          // the actual input was only 41K).
          this.stats.cacheReadTokens += usage.cacheReadTokens || 0
          this.stats.cacheWriteTokens += usage.cacheWriteTokens || 0
          this.stats.reasoningTokens += usage.reasoningTokens || 0
          const respTime = Date.now() - startTime
          this.events.onResponse?.({ path: '/v1/messages', model, status: 200, tokens: usage.inputTokens + usage.outputTokens, inputTokens: usage.inputTokens, outputTokens: usage.outputTokens, cacheReadTokens: usage.cacheReadTokens, reasoningTokens: usage.reasoningTokens, credits: usage.credits, responseTime: respTime })
          this.recordRequest({ path: '/v1/messages', model, accountId: account.id, inputTokens: usage.inputTokens, outputTokens: usage.outputTokens, credits: usage.credits, responseTime: respTime, success: true, attempts: triedAccountIds.size, accountChain: [...triedAccountIds] })
          // 记录 API Key 用量
          if (matchedApiKey) {
        ...
        (error) => {
          ...
          // Record the failure on the current account before deciding what's
          // next — the pool needs to know to put it on cooldown either way.
          this.recordRequestFailed()
          ...
          this.recordRequest({ path: '/v1/messages', model, accountId: account.id, responseTime: Date.now() - startTime, success: false, error: error.message, attempts: triedAccountIds.size, accountChain: [...triedAccountIds] })

          // Pre-output, throttle-retry path. Conditions:
          //   1. nothing has been written to the body yet beyond the initial
          //      message_start (no text deltas, tool calls, thinking blocks)
          //   2. error looks like throttle/quota — the family the upstream
          //      can recover from with a different account
          //   3. multi-account is on so we have somewhere to pivot
          //   4. we haven't run the pool more than maxPoolPasses times
          // If all hold: pick the next untried (or least-cooled) account,
          // recurse into handleClaudeStream with the SAME msgId so the client
          // sees one continuous stream — message_start will be skipped on
          // re-entry because currentRound > 0.
          const noOutputYet = !hasStartedTextBlock && !hasStartedThinkingBlock &&
            currentBlockIndex === contentBlockIndex && pendingToolCalls.size === 0
          const isThrottleLike = /\b429\b|quota|throttl|rate limit|reached the limit|ServiceQuotaExceededException|limit exceeded|TooManyRequests/i.test(error.message)
          const maxPoolPasses = 2
          if (noOutputYet && isThrottleLike && this.config.enableMultiAccount && poolPasses < maxPoolPasses) {
            // Pick the next account we haven't tried yet this request.
            const next = this.accountPool.getNextAccount(triedAccountIds)
            if (next) {
              const nextTried = new Set(triedAccountIds)
              nextTried.add(next.id)
              // Detect a full pool pass: when triedAccountIds reaches every
              // enabled account in the pool, we've completed one round.
              const enabledCount = this.accountPool.getAllAccounts().filter(a => a.enabled !== false).length
              const newPasses = nextTried.size >= enabledCount ? poolPasses + 1 : poolPasses
              console.log(`[ProxyServer] Throttle-retry stream: ${account.id.slice(0, 8)} → ${next.id.slice(0, 8)} (tried=${nextTried.size}/${enabledCount}, passes=${newPasses})`)
              // Refresh token if needed so the next call doesn't immediately
              // 401 and burn a retry slot.
              const proceed = async () => {
                try {
                  if (this.isTokenExpiringSoon(next)) {
                    const ok = await this.refreshToken(next, signal)
                    if (!ok) throw new Error('Token refresh failed for retry account')
                  }
                  const refreshed = this.accountPool.getAccount(next.id) || next
                  await this.handleClaudeStream(
                    res, refreshed, kiroPayload, model, startTime,
                    currentRound + 1, id, true, currentBlockIndex,
                    matchedApiKey, toolNameRegistry, signal,
                    simulatedCacheUsage, nextTried, newPasses,
                  )
                  resolve()
                } catch (e) {
                  // Retry chain itself blew up — fall back to error event.
                  console.error('[ProxyServer] Retry chain failed:', e)
            ...
            console.log('[ProxyServer] Throttle-retry: no untried accounts left, giving up')
          }

          // Fall-through: either output already started, or we exhausted the
          // pool. Send error to the client — Claude Code will retry from its
          // side, which is fine.
          console.error('[ProxyServer] Stream error:', error)
          const errorEvent = createClaudeStreamEvent('error', {
  ...
  private recordRequest(log: {
    ...
    if (log.success && attempts > 1) {
      this.stats.retriedRequests += 1
      this.stats.totalAccountSwitches += attempts - 1
    }
    // Pick a status code: explicit > parsed from error message > fallback.
    // Parsing the error catches our own throw new Error(`HTTP 429: ...`) form
    // so older call sites that don't pass statusCode still get a real number.
    let statusCode = log.statusCode
    if (!statusCode) {
      if (log.success) {
        statusCode = 200
      } else if (log.error) {
        const m = log.error.match(/\b(4\d{2}|5\d{2})\b/)
    ...
    this.stats.recentRequests.push({
      ...
      accountChain: attempts > 1 ? log.accountChain : undefined,
    })
    ...
    // 24h rolling window — only count successful requests (failed ones produce
    // no real usage). Events are appended in monotonic order, so prune is cheap.
    if (log.success) {
      this.stats.usage24h.push({
        timestamp: Date.now(),
        inputTokens: log.inputTokens || 0,
        outputTokens: log.outputTokens || 0,
        credits: log.credits,
