...
const DEFAULT_CONFIG: AccountPoolConfig = {
  // 节流型 429 是几秒级的瞬时错误（AWS Kiro 后端按 IP+account 维度做令牌桶），
  // 不是真"配额耗尽"。压测确认:同一账号短间隔下偶发 429,但 1-2 秒后即恢复。
  // 用 10s 短冷却,errorCount=2 时 20s,3 时 40s——账号池循环 2 轮内能再回到这个号。
  baseCooldownMs: 10000,
  // 限制最大退避,避免 errorCount 累计后冷却被打到几小时。30 倍 = 5 分钟封顶。
  maxBackoffMultiplier: 30,
  quotaResetMs: 3600000,
  // 冷却期内的概率重试:留 30%,Half-Open 状态更激进
  probabilisticRetryChance: 0.3,
}
...
export class AccountPool {
  ...
  // 连续失败 N 次直接拉黑名单（永久禁用，需手动启用）。中间有一次成功就清零。
  private blacklistThreshold: number = 5
  // Hook called when a account hits the blacklist threshold; the index layer
  // wires this to flip enabled=false in the on-disk store and remove from pool.
  private onBlacklist?: (accountId: string, reason: string) => void

  constructor(config: Partial<AccountPoolConfig> = {}) {
    this.config = { ...DEFAULT_CONFIG, ...config }
  }

  // Hook for "5 consecutive failures → permanently disable" handling. Called
  // exactly once per account per crossing of the threshold.
  setBlacklistHandler(fn: (accountId: string, reason: string) => void): void {
    this.onBlacklist = fn
  }
  ...
  recordError(accountId: string, errorType: ErrorType = ErrorType.RECOVERABLE, statusCode?: number): void {
    ...
    // 402 才是真"信用卡额度耗尽",直接月度封禁。
    // 429 是节流(throttling),压测确认 AWS 后端 1-2 秒后就放行,
    // 只用 errorCount 配合 baseCooldownMs(10s) 做短退避,不要标记成耗尽。
    if (statusCode === 402) {
    ...
    // 连续失败达到阈值 → 拉黑名单（手动启用前不再使用）。recordSuccess 会
    // 把 errorCount 重置为 0，所以这里的计数天然是"连续失败次数"。
    // 例外: 单纯 429 节流不算"账号坏掉"——AWS 后端令牌桶 1-2 秒后就放行,
    // 把它计入连续失败会冤枉超额/高频号。402 配额耗尽、403 token 失效、
    // 5xx 后端错误都算真失败。
    const isThrottle = statusCode === 429
    if (!isThrottle && errorCount >= this.blacklistThreshold) {
      console.log(`[AccountPool] Account ${account.email || accountId} hit ${errorCount} consecutive failures, blacklisting`)
      this.onBlacklist?.(accountId, `${errorCount} consecutive failures`)
    }
  }
